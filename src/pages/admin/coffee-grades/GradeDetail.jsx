...
const GradeDetail = () => {
  ...
  const fetchGradeDetails = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await AxiosInstance.get(`/permits/coffee-grades/${gradeId}/`);
      ...
      showToast(
        error.response?.data?.detail || 'Failed to fetch grade details. Please try again later.',
        "error"
      );
      setError(error.response?.data?.detail || 'Failed to fetch grade details. Please try again later.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await AxiosInstance.patch(`/permits/coffee-grades/${gradeId}/`, formData);
      setGrade(response.data);
      setIsEditing(false);
      ...
      showToast(
        error.response?.data?.detail || 'Failed to update grade. Please try again later.',
        "error"
      );
      setError(error.response?.data?.detail || 'Failed to update grade. Please try again later.');
    }
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      showToast(
        error.response?.data?.detail || 'Failed to delete grade. Please try again later.',
        "error"
      );
      setError(error.response?.data?.detail || 'Failed to delete grade. Please try again later.');
    }
  };
        ...
        <div className="bg-white shadow overflow-hidden sm:rounded-lg">
          ...
          <div className="border-t border-gray-200">
            <dl>
              ...
              <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                <dt className="text-sm font-medium text-gray-500">Weight per Bag</dt>
                <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="bg-white rounded shadow-lg p-6 max-w-sm w-full">
            ...
            <p className="mb-6">
              Are you sure you want to <span className="font-bold">delete</span> this grade? This action cannot be undone.
            </p>
